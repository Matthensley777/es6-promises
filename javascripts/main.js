...
const checForTypeMatch = function(human, pet) {
...
const checkForKidFriendly = function(human, pet) {
	...
	const isKidFriendly = ["kid-Friendly"];
	let isMatched = true;
...
loadHumans().then(function(humans) {
    ...
    Promise.all([loadDogs(), loadCats(), loadDinos()])
        .then(function(result) {
            console.log(result);
            results.forEach(function(xhrResult) {
                xhrCall.forEach(function(animal) {
                    myAnimals.push(animals);
